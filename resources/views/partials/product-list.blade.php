...
>
    ...
    <div class="catalog-card-body">
        ...
        @if($product->isOutOfStock())
            ...
            <button type="button" class="btn-add-catalog" disabled>Out of Stock</button>
        @else
            <div class="stock-badge in-stock">In Stock ({{ $product->stock_quantity }})</div>
            <button type="button" class="btn-add btn-add-catalog" data-product-id="{{ $product->id }}">Add to Cart</button>
        @endif
    </div>
</div>
@empty
